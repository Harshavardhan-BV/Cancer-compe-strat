...
i=0
for rho_Tneg in rho_arr:
    df=pd.read_csv("../raw_output/EnvEq/singlecelltype/Tneg/rho_Tneg/p_o2=1E-2-"+"{:.2E}".format(rho_Tneg)+'.csv')
    lis.append([rho_Tneg,df.o2.iloc[-1],df.Tneg.iloc[-1]])
    ax[i,0].plot(df.t/24/60,df.o2,color="tab:blue")
    ax[i,0].set_ylabel("O2 (proportion)")
    ax[i,0].set_title("rho="+"{:.2E}".format(rho_Tneg))
...
fig.savefig('../figures/EnvEq/singlecelltype/Tneg/rho_Tneg/p_o2=1E-2-timeseries.svg')
...
df1=pd.DataFrame(lis,columns=['rho_Tneg','o2_eq','Tneg_eq'])
df1.to_csv('../analysed_data/EnvEq/singlecelltype/Tneg/rho_Tneg/p_o2=1E-2-eq_values.csv',index=False)
...
fig.savefig('../figures/EnvEq/singlecelltype/Tneg/rho_Tneg/p_o2=1E-2-eq-vs-rho.svg')
