...
cases=pd.read_csv('./Tpro-Tpos_cases.csv')
parms_array=np.empty([0,7])
for i in range(len(cases)):
    for ir in initial_ratio_arr:
        for tc in tot_cell_arr:
            parms_array=np.append(parms_array,[[cases.Case[i],cases.ultestTpro[i],cases.llo2Tpro[i],cases.ultestTpos[i],cases.llo2Tpos[i],ir,tc]],axis=0)
