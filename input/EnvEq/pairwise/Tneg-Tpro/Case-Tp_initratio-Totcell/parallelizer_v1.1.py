...
parms_array=np.empty([0,7])
cases=pd.read_csv('./Tpro-Tneg_cases_v1.1.csv')
for i in range(len(cases)):
    for ir in initial_ratio_arr:
        for tc in tot_cell_arr:
            parms_array=np.append(parms_array,[[cases.Case[i],cases.ultestTpro[i],cases.llo2Tpro[i],cases.ulo2Tpro[i],cases.llo2Tneg[i],ir,tc]],axis=0)

def solve_parm(parms): #calls the solve_eq function with all default inputs other than lims
    f_name_i=f_name+"Case"+"{:.1f}".format(parms[0])+'-'+"{:.1f}".format(parms[5])+'-'+"{:.1f}".format(parms[6])
    lim[1,1,1]=parms[1]
    lim[0,1,0]=parms[2]
    lim[0,1,1]=parms[3]
    lim[0,2,0]=parms[4]
    y0[1]=parms[5]*parms[6]
    y0[2]=(1-parms[5])*parms[6]
