...
for i in range(len(cases)):
    for rat in ratios:
        for tc in totcell:
            parms_array.append([cases.loc[i],rat,tc])

def solve_parm(parms): #calls the solve_eq function with all default inputs other than lims
    f_name_i=f_name+"Case"+parms[0]['Case']+"-"+np.array2string(parms[1],separator=':')+"-{}".format(parms[2])
    ...
    ic=(parms[1]/parms[1].sum()*parms[2]).astype(np.int)
    y0[0:3]=ic
